...
| M63 (v0.24.0) | **SharpCap multi-rig (`targets`)** — drive several telescopes/PCs at once, routed by body. Each entry in `sharpcap.targets[]` runs its own listener on its own `host:port`, inherits the shared knobs and overrides host/port/bodies/buffers; a Sun candidate arms only Sun rigs, a Moon candidate only Moon rigs (independent dedup + re-arm per rig). Typical use: an Hα solar scope (Sun) on one PC + a normal scope (Moon) on another. Header readout shows the union (`🎥 ☀🌙 · N×`) with a per-rig tooltip; off-body Pushover suppression uses the union. Empty `targets` → unchanged single-rig behaviour. The trigger is body+geometry based, so differing FOV/sensors per rig don't matter for triggering | done |
